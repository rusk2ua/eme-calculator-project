...
Building one yourself doesn't mean hand-editing JSON: `python3 scripts/build_site_profile.py` is an interactive wizard that walks through location, elevation, obstructions, and noise figures, validates the result, and can render the plots immediately. If you've got a Google Maps/Earth screenshot with a pin dropped at the antenna location, Claude can read obstruction positions straight off it and turn them into real bearing/distance/height numbers — see the guide's [Building a profile](docs/SITE_PROFILE_GUIDE.md#building-a-profile) section for both.
...
| **2.5.0** | **2026-08-21** | **Site profile builder tooling.** Added `scripts/build_site_profile.py`: an interactive wizard that walks through location, elevation, obstructions, and receiver noise figures, auto-fetching what it can from USGS EPQS, writing a ready-to-use profile, validating it against the real terrain model, and offering to render the polar plots immediately. Added `src/site_survey.py` (bearing/distance from two lat/lon points or two map-screenshot pixel points; fitting a complete `line_obstructions` geometry from an antenna position plus a row's two ends; obstruction height from a shadow's length and capture date/time via `ephem`'s Sun position) plus two CLI wrappers around it, `scripts/map_pixel_to_geo.py` and `scripts/estimate_shadow_height.py` — together these support a "share a map screenshot with Claude" workflow: Claude reads pixel positions of the pin and each obstruction off the image, these tools turn them into real bearing/distance/height numbers. Deliberately scoped to deterministic geometry/trig rather than automated computer-vision obstruction detection or single-photo photogrammetry, which would need real object-detection models or camera-calibrated perspective math for accuracy that likely wouldn't beat a human field estimate anyway (see the guide's "From a map screenshot, with Claude" section for the reasoning). `docs/SITE_PROFILE_GUIDE.md` gained a "Building a profile" section covering all three paths (wizard / screenshot / by hand); the old numbered by-hand walkthrough was trimmed to the parts that remain unique (field survey practice, and the post-build validation checklist) once the wizard covers the mechanical steps. Added 9 new tests (29 total, all passing), including an end-to-end test that drives the wizard itself with a scripted answer sequence exercising every obstruction-entry method. No new dependencies (`ephem` already required). |
